...
void Extract4d::visitLine() {
    ...
    if (_refSequence != NULL) {
        if (_bedLine._version <= 9) {
            ...
            if (_bedLine._end <= _bedLine._start || _bedLine._end > (hal_index_t)_refSequence->getSequenceLength()) {
...
void Extract4d::extractBlocks4d(bool conserved) {
    ...
    for (hal_size_t i = reversed ? _bedLine._blocks.size() - 1 : 0;
         reversed ? i != (hal_size_t)-1 : i < _bedLine._blocks.size(); reversed ? --i : ++i) {
        BedBlock block = _bedLine._blocks[i];
        if (block._length == 0 ||
            _bedLine._start + block._start + block._length > (hal_index_t)_refSequence->getSequenceLength()) {
            // Bad bed block.
            ...
            if (frame == 2) {
                if (isFourfoldDegenerate(currCodonPrefix[0], currCodonPrefix[1])) {
                    if (!conserved ||
                        ...
                        BedBlock outBlock;
                        outBlock._start = dna->getArrayIndex() - _refSequence->getStartPosition() - _bedLine._start;
                        outBlock._length = 1;
                        if (reversed) {
