...
static void initParser(CLParser &optionsParser) {
    ...
    optionsParser.addArgument("refGenome", "name of reference genome (analyzed branch is "
                                           "this genome and its parent).");
    optionsParser.addOption("refFile", "bed file to write structural "
                                       "rearrangements in reference genome coordinates "
                                       "(or stdout)",
                            "\"\"");
    optionsParser.addOption("parentFile", "bed file to write rearrangements "
    ...
    optionsParser.addOption("snpFile", "bed file write point mutations to "
                            ...
                            "\"\"");
    optionsParser.addOption("delBreakFile", "bed file write deletion breakpoints to "
                                            "in reference genome coordinates (or stdout)",
                            "\"\"");
    ...
    optionsParser.addOption("length", "length of the reference genome (or sequence"
                            ...
                            0);
    optionsParser.addOption("maxGap", "maximum indel length to be considered a gap.  Gaps "
                                      " can be nested within other rearrangements.",
                            20);
    optionsParser.addOption("maxNFraction", "maximum fraction of Ns in a rearranged segment "
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        if (refBedPath == "\"\"" && parentBedPath == "\"\"" && snpBedPath == "\"\"" && delBreakBedPath == "\"\"") {
            throw hal_exception("at least one of --refFile, --parentFile, "
                                "--snpFile or --delBreakFile must be specified");
        }
        if (refTargetsPath != "\"\"" && (refTargetsPath == refBedPath || refTargetsPath == snpBedPath ||
                                         refTargetsPath == parentBedPath || refTargetsPath == delBreakBedPath)) {
            throw hal_exception("cannot output to same file as --refTargets");
        }
        if (parentBedPath != "\"\"" &&
            (parentBedPath == refBedPath || parentBedPath == snpBedPath || parentBedPath == delBreakBedPath)) {
        ...
        if (refTargetsPath != "\"\"") {
            ...
            while (!refTargetsStream.bad() && !refTargetsStream.eof()) {
                ...
                if (ss.bad() || ss.fail() || refSequenceName.empty() || start == -1 || end == (hal_size_t)-1) {
                    if (!refSequenceName.empty() && refSequenceName[0] != '#') {
                        cerr << "skipping malformed line " << line << " in " << refTargetsPath << "\n";
                    }
                } else {
                    refSequence = refGenome->getSequence(refSequenceName);
                    length = end - start;
