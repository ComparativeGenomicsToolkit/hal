...
struct GappedSegmentSimpleIteratorTest : public AlignmentTest {
    ...
    void checkCallBack(const Alignment *alignment) {
        ...
        GappedTopSegmentIteratorPtr gtsIt = child->getGappedTopSegmentIterator(0, 9999999);
        GappedBottomSegmentIteratorPtr gbsIt = parent->getGappedBottomSegmentIterator(0, 0, 9999999);
        GappedTopSegmentIteratorPtr gtsItRev = child->getGappedTopSegmentIterator(0, 9999999);
        gtsItRev->toReverse();
        GappedBottomSegmentIteratorPtr gbsItRev = parent->getGappedBottomSegmentIterator(0, 0, 9999999);
        gbsItRev->toReverse();
        ...
        for (size_t i = 0; i < child->getNumTopSegments(); ++i) {
            ...
            CuAssertTrue(_testCase, (size_t)tsItRev->getTopSegment()->getArrayIndex() == i);
            gtsItRev->toLeft();

            BottomSegmentIteratorPtr bsItRev = gbsItRev->getLeft();
            CuAssertTrue(_testCase, bsItRev->equals(gbsItRev->getRight()));
            CuAssertTrue(_testCase, (size_t)bsItRev->getBottomSegment()->getArrayIndex() == i);
            gbsItRev->toLeft();
        }

        gtsIt = child->getGappedTopSegmentIterator(child->getNumTopSegments() - 1, 9999999);
        gbsIt = parent->getGappedBottomSegmentIterator(child->getNumTopSegments() - 1, 0, 9999999);
        gtsItRev = child->getGappedTopSegmentIterator(child->getNumTopSegments() - 1, 9999999);
        gtsItRev->toReverse();
        gbsItRev = parent->getGappedBottomSegmentIterator(child->getNumTopSegments() - 1, 0, 9999999);
        gbsItRev->toReverse();
...
struct GappedSegmentSimpleIteratorTest2 : public AlignmentTest {
    ...
    void checkCallBack(const Alignment *alignment) {
        ...
        for (size_t i = 0; i < child->getNumTopSegments(); i += 5) {
            ...
            CuAssertTrue(_testCase, (size_t)tsIt->getTopSegment()->getArrayIndex() == i);
            tsIt = gtsIt->getRight();
            CuAssertTrue(_testCase, (size_t)tsIt->getTopSegment()->getArrayIndex() == i + 4);

            BottomSegmentIteratorPtr bsIt = gbsIt->getLeft();
            CuAssertTrue(_testCase, (size_t)bsIt->getBottomSegment()->getArrayIndex() == i);
            bsIt = gbsIt->getRight();
            CuAssertTrue(_testCase, (size_t)bsIt->getBottomSegment()->getArrayIndex() == i + 4);
            ...
            TopSegmentIteratorPtr tsItRev = gtsItRev->getLeft();
            CuAssertTrue(_testCase, (size_t)tsItRev->getTopSegment()->getArrayIndex() == i + 4);
            tsItRev = gtsItRev->getRight();
            CuAssertTrue(_testCase, (size_t)tsItRev->getTopSegment()->getArrayIndex() == i);
            gtsItRev->toLeft();

            BottomSegmentIteratorPtr bsItRev = gbsItRev->getLeft();
            CuAssertTrue(_testCase, (size_t)bsItRev->getBottomSegment()->getArrayIndex() == i + 4);
            bsItRev = gbsItRev->getRight();
            CuAssertTrue(_testCase, (size_t)bsItRev->getBottomSegment()->getArrayIndex() == i);
            gbsItRev->toLeft();
        }

        gtsIt = child->getGappedTopSegmentIterator(child->getNumTopSegments() - 5, 9999999);
        gbsIt = parent->getGappedBottomSegmentIterator(child->getNumTopSegments() - 5, 0, 9999999);
        gtsItRev = child->getGappedTopSegmentIterator(child->getNumTopSegments() - 5, 9999999);
        gtsItRev->toReverse();
        gbsItRev = parent->getGappedBottomSegmentIterator(child->getNumTopSegments() - 5, 0, 9999999);
        gbsItRev->toReverse();
...
struct GappedSegmentIteratorIndelTest : public AlignmentTest {
    ...
    void checkCallBack(const Alignment *alignment) {
        ...
        for (size_t i = 0; i < child->getNumTopSegments(); i += 20) {
            ...
            CuAssertTrue(_testCase, (size_t)tsIt->getTopSegment()->getArrayIndex() == i + 19);

            BottomSegmentIteratorPtr bsIt = gbsIt->getLeft();
            CuAssertTrue(_testCase, (size_t)bsIt->getBottomSegment()->getArrayIndex() == i);
            bsIt = gbsIt->getRight();
            CuAssertTrue(_testCase, (size_t)bsIt->getBottomSegment()->getArrayIndex() == i + 19);
            ...
            TopSegmentIteratorPtr tsItRev = gtsItRev->getLeft();
            CuAssertTrue(_testCase, (size_t)tsItRev->getTopSegment()->getArrayIndex() == i + 19);
            tsItRev = gtsItRev->getRight();
            CuAssertTrue(_testCase, (size_t)tsItRev->getTopSegment()->getArrayIndex() == i);
            gtsItRev->toLeft();

            BottomSegmentIteratorPtr bsItRev = gbsItRev->getLeft();
            CuAssertTrue(_testCase, (size_t)bsItRev->getBottomSegment()->getArrayIndex() == i + 19);
            bsItRev = gbsItRev->getRight();
            CuAssertTrue(_testCase, (size_t)bsItRev->getBottomSegment()->getArrayIndex() == i);
            gbsItRev->toLeft();
        }

        gtsIt = child->getGappedTopSegmentIterator(child->getNumTopSegments() - 20, 9999999);
        gbsIt = parent->getGappedBottomSegmentIterator(child->getNumTopSegments() - 20, 0, 9999999);
        gtsItRev = child->getGappedTopSegmentIterator(child->getNumTopSegments() - 20, 9999999);
        gtsItRev->toReverse();
        gbsItRev = parent->getGappedBottomSegmentIterator(child->getNumTopSegments() - 20, 0, 9999999);
        gbsItRev->toReverse();
...
static void halGappedSegmentSimpleIteratorTest(CuTest *testCase) {
    GappedSegmentSimpleIteratorTest tester;
    tester.check(testCase);
}

static void halGappedSegmentSimpleIteratorTest2(CuTest *testCase) {
    GappedSegmentSimpleIteratorTest2 tester;
    tester.check(testCase);
}

static void halGappedSegmentIteratorIndelTest(CuTest *testCase) {
    GappedSegmentIteratorIndelTest tester;
    tester.check(testCase);
}

static CuSuite *halGappedSegmentIteratorTestSuite(void) {
...
int main(int argc, char *argv[]) {
    return runHalTestSuite(argc, argv, halGappedSegmentIteratorTestSuite());
}
