...
void hal::MMapGenomeSiteMap::loadTmpTree(const vector<MMapSequence *> &sequences, struct rb_tree *tmpTree,
    ...
    for (auto seq : sequences) {
        ...
        node->_siteMapNode = MMapGenomeSiteMapNode(seq->getStartPosition(), seq->getSequenceLength(), seq->getArrayIndex());
...
hal_index_t hal::MMapGenomeSiteMap::copyTree(struct rb_tree_node *tmpNode, int &nextNodeIdx) {
    if (tmpNode == NULL) {
        return NULL_INDEX;
    }
