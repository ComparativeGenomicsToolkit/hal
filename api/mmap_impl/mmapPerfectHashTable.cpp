...
size_t hal::MMapPerfectHashTable::hashTableRelOffset(const struct phf* phf) const {
    return MMapFile::alignRound(sizeof(PerfectHashTableData))
        + MMapFile::alignRound(phf->r * sizeof(uint32_t));
}
...
void hal::MMapPerfectHashTable::writePhf(size_t phtOffset,
                                         struct phf* phf) {
...
void hal::MMapPerfectHashTable::buildHash(const vector<string>& newKeys,
    ...
    phf_error_t err = PHF::init<string, DEFAULT_PHF_NODIV>(&newPhf, &allKeys[0], allKeys.size(), DEFAULT_PHF_LAMBDA, DEFAULT_PHF_ALPHA, DEFAULT_PHF_SEED);
    if (err != 0) {
...
size_t hal::MMapPerfectHashTable::addKeys(const vector<string>& newKeys,
                                          const vector<string>& existingKeys) {
