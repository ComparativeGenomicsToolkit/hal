...
GappedTopSegmentIterator::GappedTopSegmentIterator(TopSegmentIteratorPtr leftTopSegIt, hal_size_t gapThreshold, bool atomic)
    : _gapThreshold(gapThreshold), _atomic(atomic) {
...
// SEGMENT INTERFACE
//////////////////////////////////////////////////////////////////////////////
void GappedTopSegmentIterator::setArrayIndex(Genome *genome, hal_index_t arrayIndex) {
...
const Sequence *GappedTopSegmentIterator::getSequence() const {
    assert(_left->getTopSegment()->getSequence() == _right->getTopSegment()->getSequence());
...
void GappedTopSegmentIterator::setCoordinates(hal_index_t startPos, hal_size_t length) {
...
std::ostream &GappedTopSegmentIterator::print(std::ostream &os) const {
...
void GappedTopSegmentIterator::toLeft(hal_index_t leftCutoff) {
    ...
    assert(_left->equals(_right) || _left->getReversed() || _left->leftOf(_right->getStartPosition()));
    assert(_left->equals(_right) || !_left->getReversed() || _left->rightOf(_right->getStartPosition()));
    ...
    if ((_right->getReversed() == true && i < ns) || (_right->getReversed() == false && i > 0)) {
        extendLeft();
    }
}
...
void GappedTopSegmentIterator::toRight(hal_index_t rightCutoff) {
    ...
    if ((_left->getReversed() == true && i > 0) || (_left->getReversed() == false && i < ns)) {
        extendRight();
    }
}
...
void GappedTopSegmentIterator::toSite(hal_index_t position, bool slice) {
...
void GappedTopSegmentIterator::slice(hal_offset_t startOffset, hal_offset_t endOffset) {
...
hal_size_t GappedTopSegmentIterator::getNumSegments() const {
    return (hal_size_t)abs(_right->getTopSegment()->getArrayIndex() - _left->getTopSegment()->getArrayIndex()) + 1;
}
...
GappedTopSegmentIteratorPtr GappedTopSegmentIterator::clone() const {
    GappedTopSegmentIterator *newIt = new GappedTopSegmentIterator(_left, _gapThreshold, _atomic);
...
void GappedTopSegmentIterator::copy(const GappedTopSegmentIteratorPtr &gapTopSegIt) {
...
void GappedTopSegmentIterator::toChild(const GappedBottomSegmentIteratorPtr &gapBotSegIt) {
...
bool GappedTopSegmentIterator::equals(const GappedTopSegmentIteratorPtr &other) const {
    _temp->copy(_left);
    toRightNextUngapped(_temp);
...
bool GappedTopSegmentIterator::adjacentTo(const GappedTopSegmentIteratorPtr &other) const {
    _temp->copy(_left);
    toLeftNextUngapped(_temp);
...
bool GappedTopSegmentIterator::getParentReversed() const {
    if (hasParent()) {
        ...
        assert(_temp->getTopSegment()->getParentReversed() == _temp2->getTopSegment()->getParentReversed());
...
void GappedTopSegmentIterator::setLeft(const TopSegmentIteratorPtr &topSegIt) {
...
// INTERNAL METHODS
//////////////////////////////////////////////////////////////////////////////
bool GappedTopSegmentIterator::compatible(TopSegmentIteratorPtr leftTopSegIt, TopSegmentIteratorPtr rightTopSegIt) const {
    ...
    if ((!_leftParent->getReversed() && _leftParent->leftOf(_rightParent->getStartPosition()) == false) ||
        (_leftParent->getReversed() && _leftParent->rightOf(_rightParent->getStartPosition()) == false)) {
        return false;
    }

    if (leftTopSegIt->getTopSegment()->getSequence() != rightTopSegIt->getTopSegment()->getSequence() ||
        _leftParent->getBottomSegment()->getSequence() != _rightParent->getBottomSegment()->getSequence()) {
        return false;
    }
    ...
    while (true) {
        ...
        _leftParent->toRight();
        if (_leftParent->bseg()->hasChild(_childIndex) == true || _leftParent->getLength() > _gapThreshold) {
...
void GappedTopSegmentIterator::extendRight() {
    _right->copy(_left);
    if (_atomic == true || (!_right->getReversed() && _right->getTopSegment()->isLast()) ||
    ...
    while (_right->isLast() == false) {
        ...
        if ((_right->tseg()->hasParent() == false && _right->getLength() > _gapThreshold) ||
            ...
            (_right->tseg()->hasParent() == true && _temp->tseg()->hasParent() == true && compatible(_temp, _right) == false)) {
            _right->toLeft();
            break;
        }
        _temp->toRight();
...
void GappedTopSegmentIterator::extendLeft() {
    _left->copy(_right);
    if (_atomic == true || (!_left->getReversed() && _left->getTopSegment()->isFirst()) ||
    ...
    while (_left->isFirst() == false) {
        ...
        if ((_left->tseg()->hasParent() == false && _left->getLength() > _gapThreshold) ||
            (_temp->tseg()->hasParent() == false && _temp->getLength() > _gapThreshold) ||
            (_left->tseg()->hasParent() == true && _temp->tseg()->hasParent() == true && compatible(_left, _temp) == false)) {
            _left->toRight();
            break;
        }
        _temp->toLeft();
...
void GappedTopSegmentIterator::toLeftNextUngapped(BottomSegmentIteratorPtr botSegIt) const {
...
void GappedTopSegmentIterator::toRightNextUngapped(BottomSegmentIteratorPtr botSegIt) const {
    while (botSegIt->bseg()->hasChild(_childIndex) == false && botSegIt->getLength() <= _gapThreshold) {
...
void GappedTopSegmentIterator::toLeftNextUngapped(TopSegmentIteratorPtr topSeqIt) const {
...
void GappedTopSegmentIterator::toRightNextUngapped(TopSegmentIteratorPtr topSeqIt) const {
    while (topSeqIt->tseg()->hasParent() == false && topSeqIt->getLength() <= _gapThreshold) {
