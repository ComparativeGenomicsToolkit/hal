  ...
  hal_size_t minLength)
{
  ...
  else
  {
    ...
    TopSegmentIteratorPtr topSegIt = 
    ...
    do
    {
      ...
      newSourceSegIt->slice(newSourceSegIt->getStartOffset() + startDelta, 
                            newSourceSegIt->getEndOffset() + endDelta);
...
static void clipAagainstB(MappedSegmentPtr segA,
...
static void insertAndBreakOverlaps(
  MappedSegmentPtr seg,
  MappedSegmentSet& results)
{
...
static hal_size_t mapSource(const SegmentIterator* source,
                            MappedSegmentSet& results,
                            const Genome* tgtGenome,
...
hal_size_t hal::halMapSegment(
    const SegmentIterator* source,
  ...
  const Genome *mrca)
{
  ...
  hal_size_t numResults = mapSource(source, outSegments,
                                    tgtGenome,
...
hal_size_t hal::halMapSegmentSP(
    const SegmentIteratorPtr& source,
    MappedSegmentSet& outSegments,
    const Genome* tgtGenome,
    ...
    const Genome *coalescenceLimit,
    const Genome *mrca) {
    return halMapSegment(source.get(), outSegments, tgtGenome, genomesOnPath, doDupes, minLength,
                         coalescenceLimit, mrca);
}
