...
SegmentIterator::SegmentIterator(hal_offset_t startOffset, hal_offset_t endOffset, bool reversed)
    : _startOffset(startOffset), _endOffset(endOffset), _reversed(reversed) {
}

//////////////////////////////////////////////////////////////////////////////
// SEGMENT INTERFACE
//////////////////////////////////////////////////////////////////////////////
void SegmentIterator::setArrayIndex(Genome *genome, hal_index_t arrayIndex) {
...
hal_index_t SegmentIterator::getStartPosition() const {
    ...
    if (not _reversed) {
        ...
        return getSegment()->getStartPosition() + getSegment()->getLength() - _startOffset - 1;
    }
}
...
void SegmentIterator::setCoordinates(hal_index_t startPos, hal_size_t length) {
...
std::ostream &SegmentIterator::print(ostream &os) const {
    ...
    if (ai != NULL_INDEX && !offRight) {
        os << " start=" << getStartPosition() << " end=" << getEndPosition() << " len=" << getLength() << " off=["
...
void SegmentIterator::slice(hal_offset_t startOffset, hal_offset_t endOffset) {
...
void SegmentIterator::toLeft(hal_index_t leftCutoff) {
    if (_reversed == false) {
        if (_startOffset == 0) {
            getSegment()->setArrayIndex(getGenome(), getSegment()->getArrayIndex() - 1);
            _endOffset = 0;
        } else {
            _endOffset = getSegment()->getLength() - _startOffset;
            _startOffset = 0;
        }
        if (getSegment()->getArrayIndex() >= 0 && leftCutoff != NULL_INDEX && overlaps(leftCutoff)) {
        ...
        if ((hal_size_t)getArrayIndex() < getNumSegmentsInGenome() && leftCutoff != NULL_INDEX && overlaps(leftCutoff)) {
            _startOffset = getSegment()->getStartPosition() + getSegment()->getLength() - 1 - leftCutoff;
        }
    }
    assert((hal_size_t)getArrayIndex() >= getNumSegmentsInGenome() || getArrayIndex() < 0 ||
...
void SegmentIterator::toRight(hal_index_t rightCutoff) {
    if (_reversed == false) {
        ...
        if ((hal_size_t)getArrayIndex() < getNumSegmentsInGenome() && rightCutoff != NULL_INDEX && overlaps(rightCutoff)) {
            _endOffset = getSegment()->getStartPosition() + getSegment()->getLength() - rightCutoff - 1;
        }
    }
...
void SegmentIterator::toSite(hal_index_t position, bool slice) {
    ...
    hal_index_t hint = (hal_index_t)min(nseg - 1., avgLen * ((double)position / (double)len));
    ...
    assert(getSegment()->getArrayIndex() >= 0 && getSegment()->getArrayIndex() < nseg);
    ...
    while (overlaps(position) == false) {
        ...
        if (rightOf(position)) {
            ...
            hal_index_t delta = (hal_index_t)max((rightStartPosition - position) / avgLen, 1.);
            delta = min(delta, getSegment()->getArrayIndex());
            getSegment()->setArrayIndex(genome, getSegment()->getArrayIndex() - delta);
            assert(getSegment()->getArrayIndex() >= 0 && getSegment()->getArrayIndex() < nseg);
        } else {
            ...
            avgLen = double(rightStartPosition - leftStartPosition) / (right - left);
            hal_index_t delta = (hal_index_t)max((position - leftStartPosition) / avgLen, 1.);
    ...
    if (slice) {
        ...
        _endOffset = getSegment()->getStartPosition() + getSegment()->getLength() - position - 1;
        if (_reversed) {
            // FIXME: why disabled??
            //       swap(_startOffset, _endOffset);
        }
