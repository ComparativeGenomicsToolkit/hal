...
// SEGMENT INTERFACE
//////////////////////////////////////////////////////////////////////////////
void GappedBottomSegmentIterator::setArrayIndex(Genome *genome, hal_index_t arrayIndex) {
...
const Sequence *GappedBottomSegmentIterator::getSequence() const {
    assert(_left->getBottomSegment()->getSequence() == _right->getBottomSegment()->getSequence());
...
void GappedBottomSegmentIterator::setCoordinates(hal_index_t startPos, hal_size_t length) {
...
void GappedBottomSegmentIterator::toLeft(hal_index_t leftCutoff) {
    ...
    hal_index_t ns = _right->getBottomSegment()->getGenome()->getNumBottomSegments();
...
void GappedBottomSegmentIterator::toRight(hal_index_t rightCutoff) {
    ...
    hal_index_t ns = _left->getBottomSegment()->getGenome()->getNumBottomSegments();
...
void GappedBottomSegmentIterator::toSite(hal_index_t position, bool slice) {
...
void GappedBottomSegmentIterator::slice(hal_offset_t startOffset, hal_offset_t endOffset) {
...
hal_size_t GappedBottomSegmentIterator::getNumSegments() const {
    return (hal_size_t)abs(_right->getBottomSegment()->getArrayIndex() - _left->getBottomSegment()->getArrayIndex()) + 1;
}
...
hal_size_t GappedBottomSegmentIterator::getNumGaps() const {
    ...
    for (; _temp->equals(_right) == false; _temp->toRight()) {
        if (_temp->bseg()->hasChild(_childIndex) == false && _temp->getLength() <= _gapThreshold) {
            ++count;
        }
    }
...
hal_size_t GappedBottomSegmentIterator::getNumGapBases() const {
    ...
    _temp->copy(_left);
    for (; _temp->equals(_right) == false; _temp->toRight()) {
        if (_temp->bseg()->hasChild(_childIndex) == false) {
...
GappedBottomSegmentIteratorPtr GappedBottomSegmentIterator::clone() const {
    GappedBottomSegmentIterator *gapBotSegIt = new GappedBottomSegmentIterator(_left, _childIndex, _gapThreshold, _atomic);
...
void GappedBottomSegmentIterator::copy(GappedBottomSegmentIteratorPtr gapBotSegIt) {
...
void GappedBottomSegmentIterator::toParent(GappedTopSegmentIteratorPtr gapTopSegIt) {
...
bool GappedBottomSegmentIterator::equals(GappedBottomSegmentIteratorPtr other) const {
    _temp->copy(_left);
...
bool GappedBottomSegmentIterator::adjacentTo(GappedBottomSegmentIteratorPtr other) const {
    _temp->copy(_left);
    toLeftNextUngapped(_temp);
...
bool GappedBottomSegmentIterator::hasChild() const {
    ...
    return _temp->bseg()->hasChild(_childIndex) && _temp2->bseg()->hasChild(_childIndex);
}
...
bool GappedBottomSegmentIterator::getChildReversed() const {
    if (hasChild()) {
        ...
        assert(_temp->bseg()->hasChild(_childIndex) && _temp2->bseg()->hasChild(_childIndex));
...
void GappedBottomSegmentIterator::setLeft(BottomSegmentIteratorPtr botSegIt) {
...
void GappedBottomSegmentIterator::extendRight() {
    _right->copy(_left);
    if (_atomic == true || (!_right->getReversed() && _right->getBottomSegment()->isLast()) ||
    ...
    while (_right->isLast() == false) {
        ...
        if ((_right->bseg()->hasChild(_childIndex) == false && _right->getLength() > _gapThreshold) ||
            (_temp->bseg()->hasChild(_childIndex) == false && _temp->getLength() > _gapThreshold) ||
            (_right->bseg()->hasChild(_childIndex) == true && _temp->bseg()->hasChild(_childIndex) == true &&
...
void GappedBottomSegmentIterator::extendLeft() {
    _left->copy(_right);
    if (_atomic == true || (!_left->getReversed() && _left->getBottomSegment()->isFirst()) ||
    ...
    while (_left->isFirst() == false) {
        ...
        if ((_left->bseg()->hasChild(_childIndex) == false && _left->getLength() > _gapThreshold) ||
            (_temp->bseg()->hasChild(_childIndex) == false && _temp->getLength() > _gapThreshold) ||
            (_left->bseg()->hasChild(_childIndex) == true && _temp->bseg()->hasChild(_childIndex) == true &&
