    ...
    for (hsize_t lengthIdx = 0; lengthIdx < numLengths; ++lengthIdx) {
        ...
        for (hsize_t chunkIdx = 0; chunkIdx < numSizes; ++chunkIdx) {
            ...
            try {
                ...
                myNameArray.create(&file, datasetName + "_n", nameDatatype, N, &cparms);
                ...
                for (hsize_t i = 0; i < N; ++i) {
                    ...
                    sequence.set(startPosition, seqInfo, totalTopSegments, totalBottomSegments);
                    startPosition += i * 2;
                ...
                for (hsize_t i = 0; i < N; ++i) {
                    ...
                    CuAssertTrue(testCase, sequence.getName() == genName(i, length));
                    CuAssertTrue(testCase, sequence.getStartPosition() == startPosition);
                    CuAssertTrue(testCase, sequence.getSequenceLength() == i * 2);
                    CuAssertTrue(testCase, sequence.getNumTopSegments() == i * 3);
                    CuAssertTrue(testCase, sequence.getNumBottomSegments() == i * 4);
                    startPosition += i * 2;
                }
