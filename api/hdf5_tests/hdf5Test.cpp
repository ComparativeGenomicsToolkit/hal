...
void writeNumbers(hsize_t chunkSize) {
    ...
    DataSet dataset = file.createDataSet(datasetName, datatype, dataspace, cparms);
    if (chunkSize == 0) {
        ...
        for (hsize_t i = 0; i < numChunks; ++i) {
            ...
            dataset.write(numbers + bufStart, PredType::NATIVE_HSIZE, chunkSpace, dataspace);
        }
    }
    file.close();
}
...
void checkNumbers(CuTest *testCase) {
    ...
    if (cparms.getLayout() == H5D_CHUNKED) {
        ...
        for (hsize_t i = 0; i < numChunks; ++i) {
            ...
            rdataset.read(rnumbers + bufStart, PredType::NATIVE_HSIZE, chunkSpace, rdataspace);
        }
    } else {
    ...
    for (hsize_t i = 0; i < N; ++i) {
        if (rnumbers[i] != numbers[i]) {
            cerr << i << ": " << rnumbers[i] << " (should be " << numbers[i] << ")" << endl;
        }
