...
Hdf5Sequence::Hdf5Sequence(Hdf5Genome *genome, Hdf5ExternalArray *idxArray, Hdf5ExternalArray *nameArray, hal_index_t index)
...
TopSegmentIteratorPtr Hdf5Sequence::getTopSegmentIterator(hal_index_t position) {
    hal_size_t idx = position + getTopSegmentArrayIndex();
    return _genome->getTopSegmentIterator(idx);
}

TopSegmentIteratorPtr Hdf5Sequence::getTopSegmentIterator(hal_index_t position) const {
    hal_size_t idx = position + getTopSegmentArrayIndex();
    return _genome->getTopSegmentIterator(idx);
}

BottomSegmentIteratorPtr Hdf5Sequence::getBottomSegmentIterator(hal_index_t position) {
    hal_size_t idx = position + getBottomSegmentArrayIndex();
    return _genome->getBottomSegmentIterator(idx);
}

BottomSegmentIteratorPtr Hdf5Sequence::getBottomSegmentIterator(hal_index_t position) const {
    hal_size_t idx = position + getBottomSegmentArrayIndex();
    return _genome->getBottomSegmentIterator(idx);
}
...
ColumnIteratorPtr Hdf5Sequence::getColumnIterator(const std::set<const Genome *> *targets, hal_size_t maxInsertLength,
    ...
    if (position < 0 || lastPosition >= (hal_index_t)(getStartPosition() + getSequenceLength())) {
        throw hal_exception("Hdf5Sequence::getColumnIterators: input indices (" + std::to_string(position) + ", " +
...
void Hdf5Sequence::getSubString(std::string &outString, hal_size_t start, hal_size_t length) const {
    outString.resize(length);
    DnaIteratorPtr dnaIt(getDnaIterator(start));
    dnaIt->readString(outString, length);
}

void Hdf5Sequence::setSubString(const std::string &inString, hal_size_t start, hal_size_t length) {
...
RearrangementPtr Hdf5Sequence::getRearrangement(hal_index_t position, hal_size_t gapLengthThreshold, double nThreshold,
                                                bool atomic) const {
...
GappedTopSegmentIteratorPtr Hdf5Sequence::getGappedTopSegmentIterator(hal_index_t i, hal_size_t gapThreshold,
                                                                      bool atomic) const {
...
GappedBottomSegmentIteratorPtr Hdf5Sequence::getGappedBottomSegmentIterator(hal_index_t i, hal_size_t childIdx,
...
void Hdf5Sequence::set(hal_size_t startPosition, const Sequence::Info &sequenceInfo, hal_size_t topSegmentStartIndex,
