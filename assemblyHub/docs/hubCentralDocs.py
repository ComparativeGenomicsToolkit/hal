...
def writeSnakeDocs_displays(f):
    ...
    f.write("<P>\n")
    f.write("In <em>full</em> display mode, a snake track can be decomposed into two primitive drawing elements, segments, which are the colored rectangles, and adjacencies, which are the lines connecting the segments. Segments represent subsequences of the target genome aligned to the given portion of the reference genome. Adjacencies represent the covalent bonds between the aligned subsequences of the target genome. Segments can be configured to be colored by chromosome, strand or left a single color under the <em>Select track Type</em>, <em>Alignments</em>, then <em>Block coloring method</em>.\n")
    f.write("</P>\n")
    ...
    f.write("<P>\n")
    f.write("Various mouse overs are implemented and clicking on segments navigates to the corresponding region in the target genome, making it simple to instantly switch the alignment view between reference points.\n")
    f.write("</P>\n")
...
def writeSnakeDocs_credits(f):
    ...
    f.write("<P>\n")
    f.write("The <em>snake</em> alignment display was implemented by <a href=\"emailto:braney@soe.ucsc.edu\">Brian Raney</a>.<br>\n")
