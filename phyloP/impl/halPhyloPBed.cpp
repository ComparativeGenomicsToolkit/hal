...
PhyloPBed::PhyloPBed(AlignmentConstPtr alignment, const Genome *refGenome, const Sequence *refSequence, hal_index_t start,
                     hal_size_t length, hal_size_t step, PhyloP &phyloP, ostream &outStream)
    : BedScanner(), _alignment(alignment), _refGenome(refGenome), _refSequence(refSequence), _refStart(start),
      _refLength(length), _step(step), _phyloP(phyloP), _outStream(outStream) {
    if (_refLength == 0) {
        if (_refGenome != NULL) {
...
void PhyloPBed::visitLine() {
    ...
    if (refSequence != NULL && (_refSequence == NULL || refSequence == _refSequence)) {
        ...
        hal_index_t refEnd = std::min(refStart + _refLength, (hal_index_t)refSequence->getSequenceLength());
        ...
        if (_bedVersion <= 9) {
            ...
            for (size_t i = 0; i < _bedLine._blocks.size(); ++i) {
                if (_bedLine._blocks[i]._length == 0 ||
                    _bedLine._blocks[i]._start + _bedLine._blocks[i]._length >= (hal_index_t)refSequence->getSequenceLength()) {
                    cerr << "Line " << _lineNumber << ", block " << i << ": BED coordinates invalid\n";
                } else {
                    hal_index_t start = std::max(_bedLine._blocks[i]._start, refStart);
                    hal_index_t end = std::min(_bedLine._blocks[i]._start + _bedLine._blocks[i]._length, refEnd);
                    if (end > start) {
                        _phyloP.processSequence(refSequence, start, end - start, _step);
                    }
                }
        ...
        cerr << "Line " << _lineNumber << ": BED sequence " << _bedLine._chrName << " not found in genome "
             << _refGenome->getName() << '\n';
    }
}
