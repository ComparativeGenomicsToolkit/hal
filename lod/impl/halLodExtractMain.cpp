...
static void initParser(CLParser &optionsParser) {
    ...
    optionsParser.addOption("root", "Name of root genome of tree to extract (root if "
                                    "empty)",
                            "\"\"");
    optionsParser.addOption("outTree", "Phylogenetic tree of output HAL file.  Must "
                                       "contain only genomes from the input HAL file. "
                                       "(input\'s tree used if empty)",
                            "\"\"");
    optionsParser.addOption("probeFrac", "Fraction of bases in step-interval to sample while "
                                         "looking for most aligned column.",
                            0.035);
    optionsParser.addOption("minSeqFrac", "Minumum sequence length to sample as fraction of "
                            ...
                            0.5);
    optionsParser.addOptionFlag("keepSequences", "Write the sequence strings to the output "
                                                 "file.",
                                false);
    optionsParser.addOptionFlag("allSequences", "Sample all sequences (chromsomes / contigs /"
                                                " etc.) no matter how small they are.  "
                                                "By default, small sequences may be skipped if "
                                                "they fall within the step size.",
                                false);
