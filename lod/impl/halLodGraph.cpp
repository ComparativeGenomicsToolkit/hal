...
void LodGraph::build(const Alignment *alignment, const Genome *parent, const vector<const Genome *> &children,
    ...
    for (vector<const Genome *>::const_iterator child = children.begin(); child != children.end(); ++child) {
    ...
    for (set<const Genome *>::iterator gi = _genomes.begin(); gi != _genomes.end(); ++gi) {
        scanGenome(*gi);
    }
...
void LodGraph::scanGenome(const Genome *genome) {
    ...
    for (SequenceIteratorPtr seqIt = genome->getSequenceIterator(); not seqIt->atEnd(); seqIt->toNext()) {
        ...
        if (_allSequences == true ||
            (sequence->getSequenceLength() > _minSeqLen && seqEnd - lastSampledPos > (hal_index_t)_step)) {
            for (hal_index_t pos = 0; pos < (hal_index_t)len; pos += (hal_index_t)_step) {
                ...
                double redProbFac = _probeFrac * ((double)(maxTry - minTry) / (double)sequence->getSequenceLength());
                hal_index_t numProbe = (hal_index_t)std::max(1., (double)(maxTry - minTry) * redProbFac);
                hal_index_t npMinus1 = numProbe < 2 ? numProbe : numProbe - 1;
                hal_index_t probeStep = std::max((hal_index_t)1, (maxTry - minTry) / (npMinus1));
                ...
                ColumnIteratorPtr colIt = sequence->getColumnIterator(&_genomes, 0, tryPos);
                do {
                    if (colIt->getReferenceSequencePosition() != tryPos) {
                        colIt->toSite(sequence->getStartPosition() + tryPos, sequence->getEndPosition(), true);
                    }
                    ...
                    if (bestColumn(probeStep, delta, numGenomes, minSeqLen, maxDelta, maxNumGenomes, maxMinSeqLen)) {
                        bestPos = tryPos;
                        maxDelta = delta;
                ...
                if (bestPos != NULL_INDEX) {
                    if (colIt->getReferenceSequencePosition() != bestPos) {
                        colIt->toSite(sequence->getStartPosition() + bestPos, sequence->getEndPosition(), true);
                    }
                    assert(colIt->getReferenceSequence() == sequence);
...
void LodGraph::evaluateColumn(ColumnIteratorPtr colIt, hal_size_t &outDeltaMax, hal_size_t &outNumGenomes,
            ...
            for (; dnaIt != dnaSet->end() && !breakOut; ++dnaIt) {
                ...
                if (smi != _seqMap.end()) {
                    ...
                    if (si == segmentSet->end()) {
                        ...
                        hal_size_t delta = std::min(_step, (hal_size_t)std::abs((*si)->getLeftPos() - segment.getLeftPos()));
                        if (si != segmentSet->begin()) {
                            --si;
                            delta += (hal_size_t)std::abs((*si)->getLeftPos() - segment.getLeftPos());
                        } else {
                            delta *= 2;
                        }
...
bool LodGraph::bestColumn(hal_size_t probeStep, hal_size_t delta, hal_size_t numGenomes, hal_size_t minSeqLen,
                          hal_size_t maxDelta, hal_size_t maxNumGenomes, hal_size_t maxMinSeqLen) {
...
void LodGraph::addTelomeres(const Sequence *sequence) {
    ...
    LodSegment *segment = new LodSegment(&_telomeres, sequence, sequence->getStartPosition() - 1, false);
    _telomeres.addSegment(segment);
    segSet->insert(segment);
    segment = new LodSegment(&_telomeres, sequence, sequence->getEndPosition() + 1, false);
    _telomeres.addSegment(segment);
    segSet->insert(segment);
}
...
void LodGraph::createColumn(ColumnIteratorPtr colIt) {
    ...
    for (; colMapIt != colMap->end(); ++colMapIt) {
        ...
        if (sequence->getSequenceLength() > _minSeqLen) {
            ...
            const ColumnIterator::DNASet *dnaSet = colMapIt->second;
            for (ColumnIterator::DNASet::const_iterator dnaIt = dnaSet->begin(); dnaIt != dnaSet->end(); ++dnaIt) {
                hal_index_t pos = (*dnaIt)->getArrayIndex();
...
void LodGraph::optimizeByExtension() {
    ...
    for (BlockIterator bi = _blocks.begin(); bi != _blocks.end() && (*bi)->getNumSegments() > 1; ++bi) {
        (*bi)->extend(0.5);
    }
    ...
    for (BlockIterator bi = mergeList.begin(); bi != mergeList.end(); ++bi) {
        ...
        if (adjBlock != NULL) {
            for (hal_size_t segIdx = 0; segIdx < adjBlock->getNumSegments(); ++segIdx) {
                ...
                LodSegment *seg = const_cast<LodSegment *>(adjBlock->getSegment(segIdx));
...
void LodGraph::printDimensions(ostream &os) const {
    ...
    os << "Graph: numBlocks=" << _blocks.size() << " numSegs=" << totalSegments << " minSegs=" << minSegments
       << " maxSegs=" << maxSegments << " totLen=" << totalLength << " minLen=" << minLength << " maxLen=" << maxLength
       << " totAdjLen=" << totalAdjLength << " minAdjLen=" << minAdjLength << " maxAdjLen=" << maxAdjLength << endl;
}
...
bool LodGraph::checkCoverage() const {
    ...
    for (genomeIt = _genomes.begin(); genomeIt != _genomes.end(); ++genomeIt) {
        ...
        covPair.second = new vector<bool>(covPair.first->getSequenceLength(), false);
        covMap.insert(covPair);
    }

    // iterate through every segment in every block.
    for (BlockConstIterator bi = _blocks.begin(); bi != _blocks.end() && success; ++bi) {
        for (hal_size_t segIdx = 0; segIdx < (*bi)->getNumSegments() && success; ++segIdx) {
            ...
            for (hal_index_t pos = segment->getLeftPos(); pos <= segment->getRightPos(); ++pos) {
    ...
    for (covMapIt = covMap.begin(); covMapIt != covMap.end(); ++covMapIt) {
        for (SequenceIteratorPtr seqIt = covMapIt->first->getSequenceIterator(); (not seqIt->atEnd()) && success;
             seqIt->toNext()) {
            const Sequence *sequence = seqIt->getSequence();
            if (sequence->getSequenceLength() > 0) {
                hal_size_t numCovered = 0;
                for (hal_index_t i = sequence->getStartPosition(); i <= sequence->getEndPosition(); ++i) {
                    if (covMapIt->second->at(i) != true) {
                        ++numCovered;
                    }
                }
                if (numCovered != 0 && numCovered != sequence->getSequenceLength()) {
                    cerr << numCovered << " out of " << sequence->getSequenceLength() << " bases covered for sequence "
                         << sequence->getFullName() << endl;
                    success = false;
